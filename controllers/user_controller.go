...
func (uc UserController) registerRoutes(srv *Server) {
	userEndpoint := srv.Group("user")
	{
		...
		userEndpoint.GET("/role/:id", func(c echo.Context) error {
			return handleFindUsersWithRole(c, srv.DB)
		})
...
func handleFindUsersWithRole(c echo.Context, db *ent.Client) error {
	ctx := c.Request().Context()
	rawRoleId := c.Param("id")
	...
	resp, err := services.FindUsersWithRole(ctx, db, roleId)
