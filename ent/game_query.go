...
func (gq *GameQuery) QueryGames() *ServerQuery {
	query := (&ServerClient{config: gq.config}).Query()
...
func (gq *GameQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (gq *GameQuery) WithGames(opts ...func(*ServerQuery)) *GameQuery {
	query := (&ServerClient{config: gq.config}).Query()
...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withGames; query != nil {
		if err := gq.loadGames(ctx, query, nodes,
			func(n *Game) { n.Edges.Games = []*Server{} },
			func(n *Game, e *Server) { n.Edges.Games = append(n.Edges.Games, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GameQuery) loadGames(ctx context.Context, query *ServerQuery, nodes []*Game, init func(*Game), assign func(*Game, *Server)) error {
	...
	query.Where(predicate.Server(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(game.GamesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.game_games
		if fk == nil {
			return fmt.Errorf(`foreign-key "game_games" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "game_games" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
