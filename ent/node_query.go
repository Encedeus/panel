...
func (nq *NodeQuery) QueryNodes() *ServerQuery {
...
func (nq *NodeQuery) WithNodes(opts ...func(*ServerQuery)) *NodeQuery {
	query := (&ServerClient{config: nq.config}).Query()
...
func (nq *NodeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Node, error) {
	...
	if query := nq.withNodes; query != nil {
		if err := nq.loadNodes(ctx, query, nodes,
			func(n *Node) { n.Edges.Nodes = []*Server{} },
			func(n *Node, e *Server) { n.Edges.Nodes = append(n.Edges.Nodes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (nq *NodeQuery) loadNodes(ctx context.Context, query *ServerQuery, nodes []*Node, init func(*Node), assign func(*Node, *Server)) error {
	...
	for _, n := range neighbors {
		fk := n.node_nodes
		if fk == nil {
			return fmt.Errorf(`foreign-key "node_nodes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "node_nodes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
