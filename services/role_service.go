...
func FindAllRoles(ctx context.Context, db *ent.Client) (*protoapi.RoleFindAllResponse, error) {
	roles, err := db.Role.Query().Where(role.DeletedAtIsNil()).All(ctx)
	if err != nil {
		return nil, err
	}

	resp := &protoapi.RoleFindAllResponse{
		Roles: proto.EntRoleEntityToProtoRoles(roles),
	}
